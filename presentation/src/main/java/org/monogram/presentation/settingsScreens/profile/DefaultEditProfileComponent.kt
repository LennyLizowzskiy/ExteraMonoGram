...
class DefaultEditProfileComponent(
    ...
    override fun onUpdatePersonalChatId(chatId: Long) {
        _state.value = _state.value.copy(personalChatId = chatId)
        if (chatId != 0L) {
            scope.launch {
                val chat = chatsListRepository.getChatById(chatId)
                _state.value = _state.value.copy(linkedChat = chat)
            }
        } else {
            _state.value = _state.value.copy(linkedChat = null)
        }
    }

    override fun onUpdateBusinessBio(bio: String) {
        _state.value = _state.value.copy(businessBio = bio)
    }
    ...
    override fun onUpdateBusinessOpeningHours(openingHours: BusinessOpeningHoursModel?) {
        _state.value = _state.value.copy(businessOpeningHours = openingHours)
    }
    ...
    override fun onShowAvatarPicker(show: Boolean) {
        _state.value = _state.value.copy(showAvatarPicker = show)
    }
    ...
    override fun onReorderUsernames(usernames: List<String>) {
        scope.launch {
            try {
                ...
                val me = userRepository.getMe()
                _state.value = _state.value.copy(user = me, username = me.username ?: "")
            } catch (e: Exception) {
                _state.value = _state.value.copy(error = e.message)
            }
        }
